...
//        LogUtil.eTag("666","register"+tag + "  size:" + subjectList.size());
        return subject;
    }
    ...
    public RxBus unregister(@NonNull Object tag,
        ...
        if (null != subjects) {
            ...
            if (isEmpty(subjects)) {
                subjectMapper.remove(tag);
//                LogUtil.eTag("666","unregister"+ tag + "  size:" + subjects.size());
            }
        }
        return getInstance();
    }
...
//        LogUtil.eTag("666","post"+ "eventName: " + tag);
        List<Subject> subjectList = subjectMapper.get(tag);
        if (!isEmpty(subjectList)) {
            for (Subject subject : subjectList) {
                subject.onNext(content);
//                LogUtil.eTag("666","onEvent"+ "eventName: " + tag);
            }
        }
    }
